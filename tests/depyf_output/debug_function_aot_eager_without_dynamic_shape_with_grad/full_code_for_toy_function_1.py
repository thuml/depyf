...
___check_global_state = '''<built-in method check of torch._C._dynamo.guards.GlobalStateGuard object at 0x1221d9db0>'''
___check_torch_function_mode_stack = '''<function make_torch_function_mode_stack_guard.<locals>.check_torch_function_mode_stack at 0x13428c040>'''
IsNonOverlappingAndDenseIndicator = '''<function eval_is_non_overlapping_and_dense at 0x121750a40>'''
cast_symbool_to_symint_guardless = '''<function cast_symbool_to_symint_guardless at 0x121750c20>'''
...
___key_to_id = '''<function key_to_id at 0x12189c540>'''
___dict_version = '''<built-in function dict_version>'''
___dict_contains = '''<function _get_closure_vars.<locals>.<lambda> at 0x134267600>'''
___tuple_iterator_len = '''<method '__length_hint__' of 'tuple_iterator' objects>'''
___normalize_range_iter = '''<function normalize_range_iter at 0x12188be20>'''
___tuple_iterator_getitem = '''<function tuple_iterator_getitem at 0x12188bce0>'''
___dataclass_fields = '''<function dataclass_fields at 0x12188bd80>'''
___namedtuple_fields = '''<function _get_closure_vars.<locals>.<lambda> at 0x1342676a0>'''
___get_torch_function_mode_stack_at = '''<function get_torch_function_mode_stack_at at 0x1218940e0>'''
__math_isnan = '''<built-in function isnan>'''
__numpy_isnan = '''<ufunc 'isnan'>'''
inf = '''inf'''
__load_module = '''<function import_module at 0x102a66840>'''
...
___from_numpy = '''<function from_numpy at 0x121d7cae0>'''
___as_tensor = '''<function _as_tensor_fullprec at 0x117cdae80>'''
...
def __guard_1_for_toy_function(L, G, **___kwargs_ignored):
    ...
    __guard_hit = __guard_hit and ___check_obj_id(G['torch'], 4356374992)                       # x = a / (torch.abs(a) + 1)  # data/DeepLearning/depyf/tests/test_pytorch/test_pytorch.py:4 in toy_function
    __guard_hit = __guard_hit and ___check_obj_id(G['torch'].abs, 4605403568)                   # x = a / (torch.abs(a) + 1)  # data/DeepLearning/depyf/tests/test_pytorch/test_pytorch.py:4 in toy_function
    return __guard_hit

# Note: please refer to the graph code in __compiled_fn_1_ab1a8abf_ef00_4e5b_b16d_788de3ae96c7*.py.
# Captured Graph: Dynamo generated graph (debuggable when using eager backend).
# Joint graph: joint forward+backward graph from aot autograd.
# Forward graph: forward graph from aot autograd (debuggable when using aot_eager backend).
# Backward graph: backward graph from aot autograd (debuggable when using aot_eager backend).
# AFTER XXX: graph processed by inductor (not debuggable).
def __compiled_fn_1_ab1a8abf_ef00_4e5b_b16d_788de3ae96c7(*args, **kwargs):
    pass
...
def __transformed_code_1_for_toy_function(a, b):
    ...
    graph_out_0 = __compiled_fn_1_ab1a8abf_ef00_4e5b_b16d_788de3ae96c7(a, b)
    []
    __temp_7 = []
    __temp_7.extend([])
    [__temp_7]
