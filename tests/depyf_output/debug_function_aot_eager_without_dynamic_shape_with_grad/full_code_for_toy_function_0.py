...
___check_global_state = '''<built-in method check of torch._C._dynamo.guards.GlobalStateGuard object at 0x1223db1f0>'''
___check_torch_function_mode_stack = '''<function make_torch_function_mode_stack_guard.<locals>.check_torch_function_mode_stack at 0x134694040>'''
...
___key_to_id = '''<function key_to_id at 0x1218a8540>'''
___dict_version = '''<built-in function dict_version>'''
___dict_contains = '''<function _get_closure_vars.<locals>.<lambda> at 0x134663600>'''
...
___namedtuple_fields = '''<function _get_closure_vars.<locals>.<lambda> at 0x1346636a0>'''
...
inf = '''inf'''
__load_module = '''<function import_module at 0x1045b2840>'''
...
___from_numpy = '''<function from_numpy at 0x121ff8ae0>'''
___as_tensor = '''<function _as_tensor_fullprec at 0x1175d6e80>'''
...
def __guard_0_for_toy_function(L, G, **___kwargs_ignored):
    ...
    __guard_hit = __guard_hit and ___check_obj_id(G['torch'], 4375445968)                       # x = a / (torch.abs(a) + 1)  # data/DeepLearning/depyf/tests/test_pytorch/test_pytorch.py:4 in toy_function
    __guard_hit = __guard_hit and ___check_obj_id(G['torch'].abs, 4377157904)                   # x = a / (torch.abs(a) + 1)  # data/DeepLearning/depyf/tests/test_pytorch/test_pytorch.py:4 in toy_function
    return __guard_hit

# Note: please refer to the graph code in __compiled_fn_1_b6d9134b_6751_4a27_971c_f7d401d4323b*.py.
# Captured Graph: Dynamo generated graph (debuggable when using eager backend).
# Joint graph: joint forward+backward graph from aot autograd.
# Forward graph: forward graph from aot autograd (debuggable when using aot_eager backend).
# Backward graph: backward graph from aot autograd (debuggable when using aot_eager backend).
# AFTER XXX: graph processed by inductor (not debuggable).
def __compiled_fn_1_b6d9134b_6751_4a27_971c_f7d401d4323b(*args, **kwargs):
    pass
