...
___check_global_state = '''<built-in method check of torch._C._dynamo.guards.GlobalStateGuard object at 0x110936af0>'''
___check_torch_function_mode_stack = '''<function make_torch_function_mode_stack_guard.<locals>.check_torch_function_mode_stack at 0x110d6aa20>'''
IsNonOverlappingAndDenseIndicator = '''<function eval_is_non_overlapping_and_dense at 0x10dd54a40>'''
cast_symbool_to_symint_guardless = '''<function cast_symbool_to_symint_guardless at 0x10dd54c20>'''
...
___key_to_id = '''<function key_to_id at 0x10dea0540>'''
___dict_version = '''<built-in function dict_version>'''
___dict_contains = '''<function _get_closure_vars.<locals>.<lambda> at 0x110d94f40>'''
___tuple_iterator_len = '''<method '__length_hint__' of 'tuple_iterator' objects>'''
___normalize_range_iter = '''<function normalize_range_iter at 0x10de8be20>'''
___tuple_iterator_getitem = '''<function tuple_iterator_getitem at 0x10de8bce0>'''
___dataclass_fields = '''<function dataclass_fields at 0x10de8bd80>'''
___namedtuple_fields = '''<function _get_closure_vars.<locals>.<lambda> at 0x110d94fe0>'''
___get_torch_function_mode_stack_at = '''<function get_torch_function_mode_stack_at at 0x10debc0e0>'''
__math_isnan = '''<built-in function isnan>'''
__numpy_isnan = '''<ufunc 'isnan'>'''
inf = '''inf'''
__load_module = '''<function import_module at 0x102956840>'''
...
___from_numpy = '''<function from_numpy at 0x10e5f8ae0>'''
___as_tensor = '''<function _as_tensor_fullprec at 0x10c2dae80>'''
...
def __guard_1_for_forward(L, G, **___kwargs_ignored):
    ...
    __guard_hit = __guard_hit and ___check_obj_id(G['torch'], 4345758160)                       # x = a / (torch.abs(a) + 1)  # data/DeepLearning/depyf/tests/test_pytorch/test_pytorch.py:13 in forward
    __guard_hit = __guard_hit and ___check_obj_id(G['torch'].abs, 4347486640)                   # x = a / (torch.abs(a) + 1)  # data/DeepLearning/depyf/tests/test_pytorch/test_pytorch.py:13 in forward
    return __guard_hit

# Note: please refer to the graph code in __compiled_fn_1_0470f580_13ba_475e_8d4a_94da2652bf0f*.py.
# Captured Graph: Dynamo generated graph (debuggable when using eager backend).
# Joint graph: joint forward+backward graph from aot autograd.
# Forward graph: forward graph from aot autograd (debuggable when using aot_eager backend).
# Backward graph: backward graph from aot autograd (debuggable when using aot_eager backend).
# AFTER XXX: graph processed by inductor (not debuggable).
def __compiled_fn_1_0470f580_13ba_475e_8d4a_94da2652bf0f(*args, **kwargs):
    pass
...
def __transformed_code_1_for_forward(self, a, b):
    ...
    graph_out_0 = __compiled_fn_1_0470f580_13ba_475e_8d4a_94da2652bf0f(a, b)
    []
    __temp_7 = []
    __temp_7.extend([])
    [__temp_7]
