...
___check_global_state = '''<built-in method check of torch._C._dynamo.guards.GlobalStateGuard object at 0x1141678b0>'''
___check_torch_function_mode_stack = '''<function make_torch_function_mode_stack_guard.<locals>.check_torch_function_mode_stack at 0x115727a60>'''
IsNonOverlappingAndDenseIndicator = '''<function eval_is_non_overlapping_and_dense at 0x10ed54a40>'''
cast_symbool_to_symint_guardless = '''<function cast_symbool_to_symint_guardless at 0x10ed54c20>'''
...
___key_to_id = '''<function key_to_id at 0x10ee94540>'''
___dict_version = '''<built-in function dict_version>'''
___dict_contains = '''<function _get_closure_vars.<locals>.<lambda> at 0x115a647c0>'''
...
___namedtuple_fields = '''<function _get_closure_vars.<locals>.<lambda> at 0x115a64860>'''
___get_torch_function_mode_stack_at = '''<function get_torch_function_mode_stack_at at 0x10ee9c0e0>'''
__math_isnan = '''<built-in function isnan>'''
__numpy_isnan = '''<ufunc 'isnan'>'''
inf = '''inf'''
__load_module = '''<function import_module at 0x10252e840>'''
...
___from_numpy = '''<function from_numpy at 0x10f5f8ae0>'''
___as_tensor = '''<function _as_tensor_fullprec at 0x10d3d6e80>'''
...
def __guard_1_for_forward(L, G, **___kwargs_ignored):
    ...
    __guard_hit = __guard_hit and ___check_obj_id(G['torch'], 4341678544)                       # x = a / (torch.abs(a) + 1)  # data/DeepLearning/depyf/tests/test_pytorch/test_pytorch.py:13 in forward
    __guard_hit = __guard_hit and ___check_obj_id(G['torch'].abs, 4346323216)                   # x = a / (torch.abs(a) + 1)  # data/DeepLearning/depyf/tests/test_pytorch/test_pytorch.py:13 in forward
    return __guard_hit

# Note: please refer to the graph code in __compiled_fn_1_45e8518e_e2db_4ae2_b953_84ff58ed6ad6*.py.
# Captured Graph: Dynamo generated graph (debuggable when using eager backend).
# Joint graph: joint forward+backward graph from aot autograd.
# Forward graph: forward graph from aot autograd (debuggable when using aot_eager backend).
# Backward graph: backward graph from aot autograd (debuggable when using aot_eager backend).
# AFTER XXX: graph processed by inductor (not debuggable).
def __compiled_fn_1_45e8518e_e2db_4ae2_b953_84ff58ed6ad6(*args, **kwargs):
    pass
...
def __transformed_code_1_for_forward(self, a, b):
    ...
    graph_out_0 = __compiled_fn_1_45e8518e_e2db_4ae2_b953_84ff58ed6ad6(a, b)
    []
    __temp_7 = []
    __temp_7.extend([])
    [__temp_7]
