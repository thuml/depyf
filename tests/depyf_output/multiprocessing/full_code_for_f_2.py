...
___check_global_state = '''<built-in method check of torch._C._dynamo.guards.GlobalStateGuard object at 0x12d890330>'''
___check_torch_function_mode_stack = '''<function make_torch_function_mode_stack_guard.<locals>.check_torch_function_mode_stack at 0x12dab0f40>'''
IsNonOverlappingAndDenseIndicator = '''<function eval_is_non_overlapping_and_dense at 0x112800cc0>'''
cast_symbool_to_symint_guardless = '''<function cast_symbool_to_symint_guardless at 0x112800ea0>'''
...
___key_to_id = '''<function key_to_id at 0x11293c680>'''
___dict_version = '''<built-in function dict_version>'''
___dict_contains = '''<function _get_closure_vars.<locals>.<lambda> at 0x12dab09a0>'''
___tuple_iterator_len = '''<method '__length_hint__' of 'tuple_iterator' objects>'''
___normalize_range_iter = '''<function normalize_range_iter at 0x112933f60>'''
___tuple_iterator_getitem = '''<function tuple_iterator_getitem at 0x112933e20>'''
___dataclass_fields = '''<function dataclass_fields at 0x112933ec0>'''
___namedtuple_fields = '''<function _get_closure_vars.<locals>.<lambda> at 0x12dab0a40>'''
___get_torch_function_mode_stack_at = '''<function get_torch_function_mode_stack_at at 0x11296c220>'''
__math_isnan = '''<built-in function isnan>'''
__numpy_isnan = '''<ufunc 'isnan'>'''
inf = '''inf'''
__load_module = '''<function import_module at 0x100d5e840>'''
...
___from_numpy = '''<function from_numpy at 0x1130b2340>'''
___as_tensor = '''<function _as_tensor_fullprec at 0x1104ba520>'''
...
def __guard_2_for_f(L, G, **___kwargs_ignored):
    __guard_hit = True
...
# Note: please refer to the graph code in __compiled_fn_1_9d0cdcc6_06c9_4ecf_b8f0_8944635da2e6*.py.
# Captured Graph: Dynamo generated graph (debuggable when using eager backend).
# Joint graph: joint forward+backward graph from aot autograd.
# Forward graph: forward graph from aot autograd (debuggable when using aot_eager backend).
# Backward graph: backward graph from aot autograd (debuggable when using aot_eager backend).
# AFTER XXX: graph processed by inductor (not debuggable).
def __compiled_fn_1_9d0cdcc6_06c9_4ecf_b8f0_8944635da2e6(*args, **kwargs):
    pass
...
def transformed_f(x):
    ...
    if __guard_2_for_f(__local_dict, __global_dict):
        return __transformed_code_2_for_f(x)
