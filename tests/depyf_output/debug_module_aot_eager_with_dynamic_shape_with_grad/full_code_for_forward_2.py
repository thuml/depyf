...
___check_global_state = '''<built-in method check of torch._C._dynamo.guards.GlobalStateGuard object at 0x10e4edd70>'''
___check_torch_function_mode_stack = '''<function make_torch_function_mode_stack_guard.<locals>.check_torch_function_mode_stack at 0x11ee8fe20>'''
IsNonOverlappingAndDenseIndicator = '''<function eval_is_non_overlapping_and_dense at 0x10cb54a40>'''
cast_symbool_to_symint_guardless = '''<function cast_symbool_to_symint_guardless at 0x10cb54c20>'''
...
___key_to_id = '''<function key_to_id at 0x10cda0540>'''
___dict_version = '''<built-in function dict_version>'''
___dict_contains = '''<function _get_closure_vars.<locals>.<lambda> at 0x11eca4c20>'''
___tuple_iterator_len = '''<method '__length_hint__' of 'tuple_iterator' objects>'''
___normalize_range_iter = '''<function normalize_range_iter at 0x10cd8fe20>'''
___tuple_iterator_getitem = '''<function tuple_iterator_getitem at 0x10cd8fce0>'''
___dataclass_fields = '''<function dataclass_fields at 0x10cd8fd80>'''
___namedtuple_fields = '''<function _get_closure_vars.<locals>.<lambda> at 0x11eca4cc0>'''
___get_torch_function_mode_stack_at = '''<function get_torch_function_mode_stack_at at 0x10cd9c0e0>'''
__math_isnan = '''<built-in function isnan>'''
__numpy_isnan = '''<ufunc 'isnan'>'''
inf = '''inf'''
__load_module = '''<function import_module at 0x102eea840>'''
...
___from_numpy = '''<function from_numpy at 0x10d4f8ae0>'''
___as_tensor = '''<function _as_tensor_fullprec at 0x10b0dae80>'''
...
def __guard_5_for_forward(L, G, **___kwargs_ignored):
    ...
    __guard_hit = __guard_hit and L['b'].size()[0] == L['a'].size()[0]  # (unknown var s52, please file a bug)
    __guard_hit = __guard_hit and 2 <= L['a'].size()[0]  # b = b * -1 + b.sum()  # data/DeepLearning/depyf/tests/test_pytorch/test_pytorch.py:14 in forward (user code shown is first use of this value--the guard itself is not due user code but due to 0/1 specialization in the framework; to avoid specialization try torch._dynamo.decorators.mark_unbacked(tensor, dim))
    __guard_hit = __guard_hit and ___check_obj_id(G['torch'], 4352049616)                       # x = a / (torch.abs(a) + 1)  # data/DeepLearning/depyf/tests/test_pytorch/test_pytorch.py:13 in forward
    __guard_hit = __guard_hit and ___check_obj_id(G['torch'].abs, 4353614096)                   # x = a / (torch.abs(a) + 1)  # data/DeepLearning/depyf/tests/test_pytorch/test_pytorch.py:13 in forward
    return __guard_hit

# Note: please refer to the graph code in __compiled_fn_3_cfdde4b9_de2c_4e11_adda_a61054d7e6be*.py.
# Captured Graph: Dynamo generated graph (debuggable when using eager backend).
# Joint graph: joint forward+backward graph from aot autograd.
# Forward graph: forward graph from aot autograd (debuggable when using aot_eager backend).
# Backward graph: backward graph from aot autograd (debuggable when using aot_eager backend).
# AFTER XXX: graph processed by inductor (not debuggable).
def __compiled_fn_3_cfdde4b9_de2c_4e11_adda_a61054d7e6be(*args, **kwargs):
    pass
...
___check_tensors = '''None'''
___check_tensors_verbose = '''None'''
___check_global_state = '''<built-in method check of torch._C._dynamo.guards.GlobalStateGuard object at 0x10de465f0>'''
___check_torch_function_mode_stack = '''<function make_torch_function_mode_stack_guard.<locals>.check_torch_function_mode_stack at 0x11eca56c0>'''
IsNonOverlappingAndDenseIndicator = '''<function eval_is_non_overlapping_and_dense at 0x10cb54a40>'''
cast_symbool_to_symint_guardless = '''<function cast_symbool_to_symint_guardless at 0x10cb54c20>'''
math = '''<module 'math' (built-in)>'''
torch = '''<module 'torch' from '/Users/youkaichao/uv_envs/py312/lib/python3.12/site-packages/torch/__init__.py'>'''
...
# Note: please refer to the graph code in __compiled_fn_1_c686e718_751c_4585_bfe1_b0477dc93f3f*.py.
# Captured Graph: Dynamo generated graph (debuggable when using eager backend).
# Joint graph: joint forward+backward graph from aot autograd.
# Forward graph: forward graph from aot autograd (debuggable when using aot_eager backend).
# Backward graph: backward graph from aot autograd (debuggable when using aot_eager backend).
# AFTER XXX: graph processed by inductor (not debuggable).
def __compiled_fn_1_c686e718_751c_4585_bfe1_b0477dc93f3f(*args, **kwargs):
    pass
...
def __transformed_code_2_for_forward(self, a, b):
    ...
    __import_torch_dot__dynamo_dot_utils.record_pregraph_bytecode_exit(tmp_2)
    graph_out_0 = __compiled_fn_1_c686e718_751c_4585_bfe1_b0477dc93f3f(a, b)
    []
    __temp_7 = []
    __temp_7.extend([])
    [__temp_7]
