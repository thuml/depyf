...
___check_global_state = '''<built-in method check of torch._C._dynamo.guards.GlobalStateGuard object at 0x1105819f0>'''
___check_torch_function_mode_stack = '''<function make_torch_function_mode_stack_guard.<locals>.check_torch_function_mode_stack at 0x111527b00>'''
IsNonOverlappingAndDenseIndicator = '''<function eval_is_non_overlapping_and_dense at 0x10ed50a40>'''
cast_symbool_to_symint_guardless = '''<function cast_symbool_to_symint_guardless at 0x10ed50c20>'''
...
___key_to_id = '''<function key_to_id at 0x10eea8540>'''
___dict_version = '''<built-in function dict_version>'''
___dict_contains = '''<function _get_closure_vars.<locals>.<lambda> at 0x1115271a0>'''
___tuple_iterator_len = '''<method '__length_hint__' of 'tuple_iterator' objects>'''
___normalize_range_iter = '''<function normalize_range_iter at 0x10ee8be20>'''
___tuple_iterator_getitem = '''<function tuple_iterator_getitem at 0x10ee8bce0>'''
___dataclass_fields = '''<function dataclass_fields at 0x10ee8bd80>'''
___namedtuple_fields = '''<function _get_closure_vars.<locals>.<lambda> at 0x111527240>'''
___get_torch_function_mode_stack_at = '''<function get_torch_function_mode_stack_at at 0x10ee940e0>'''
__math_isnan = '''<built-in function isnan>'''
__numpy_isnan = '''<ufunc 'isnan'>'''
inf = '''inf'''
__load_module = '''<function import_module at 0x102702840>'''
...
___from_numpy = '''<function from_numpy at 0x10f5f4ae0>'''
___as_tensor = '''<function _as_tensor_fullprec at 0x10d1d6e80>'''
...
def __guard_2_for_toy_function(L, G, **___kwargs_ignored):
    ...
    __guard_hit = __guard_hit and ___check_obj_id(G['torch'], 4343202256)                       # x = a / (torch.abs(a) + 1)  # data/DeepLearning/depyf/tests/test_pytorch/test_pytorch.py:4 in toy_function
    __guard_hit = __guard_hit and ___check_obj_id(G['torch'].abs, 4345061648)                   # x = a / (torch.abs(a) + 1)  # data/DeepLearning/depyf/tests/test_pytorch/test_pytorch.py:4 in toy_function
    return __guard_hit

# Note: please refer to the graph code in __compiled_fn_1_60eca021_5901_467b_b3f1_f720cd1005aa*.py.
# Captured Graph: Dynamo generated graph (debuggable when using eager backend).
# Joint graph: joint forward+backward graph from aot autograd.
# Forward graph: forward graph from aot autograd (debuggable when using aot_eager backend).
# Backward graph: backward graph from aot autograd (debuggable when using aot_eager backend).
# AFTER XXX: graph processed by inductor (not debuggable).
def __compiled_fn_1_60eca021_5901_467b_b3f1_f720cd1005aa(*args, **kwargs):
    pass
...
def __transformed_code_2_for_toy_function(a, b):
    ...
    graph_out_0 = __compiled_fn_1_60eca021_5901_467b_b3f1_f720cd1005aa(a, b)
    []
    __temp_7 = []
    __temp_7.extend([])
    [__temp_7]
