...
___check_global_state = '''<built-in method check of torch._C._dynamo.guards.GlobalStateGuard object at 0x1385de1b0>'''
___check_torch_function_mode_stack = '''<function make_torch_function_mode_stack_guard.<locals>.check_torch_function_mode_stack at 0x13e325d00>'''
IsNonOverlappingAndDenseIndicator = '''<function eval_is_non_overlapping_and_dense at 0x11df50a40>'''
cast_symbool_to_symint_guardless = '''<function cast_symbool_to_symint_guardless at 0x11df50c20>'''
...
___key_to_id = '''<function key_to_id at 0x11e0a8540>'''
___dict_version = '''<built-in function dict_version>'''
___dict_contains = '''<function _get_closure_vars.<locals>.<lambda> at 0x13b2ff1a0>'''
___tuple_iterator_len = '''<method '__length_hint__' of 'tuple_iterator' objects>'''
___normalize_range_iter = '''<function normalize_range_iter at 0x11e08be20>'''
___tuple_iterator_getitem = '''<function tuple_iterator_getitem at 0x11e08bce0>'''
___dataclass_fields = '''<function dataclass_fields at 0x11e08bd80>'''
___namedtuple_fields = '''<function _get_closure_vars.<locals>.<lambda> at 0x13b2ff240>'''
___get_torch_function_mode_stack_at = '''<function get_torch_function_mode_stack_at at 0x11e0940e0>'''
__math_isnan = '''<built-in function isnan>'''
__numpy_isnan = '''<ufunc 'isnan'>'''
inf = '''inf'''
__load_module = '''<function import_module at 0x100dfe840>'''
...
___from_numpy = '''<function from_numpy at 0x11e6c8ae0>'''
___as_tensor = '''<function _as_tensor_fullprec at 0x11c5d6e80>'''
...
def __guard_3_for_toy_function(L, G, **___kwargs_ignored):
    ...
    __guard_hit = __guard_hit and L['b'].size()[0] == L['a'].size()[0]  # (unknown var s52, please file a bug)
    __guard_hit = __guard_hit and 2 <= L['a'].size()[0] <= 10485760  # b = b * -1 + b.sum()  # data/DeepLearning/depyf/tests/test_pytorch/test_pytorch.py:5 in toy_function (user code shown is first use of this value--the guard itself is not due user code but due to 0/1 specialization in the framework; to avoid specialization try torch._dynamo.decorators.mark_unbacked(tensor, dim)) and (_inductor/codegen/cpp.py:2213 in need_use_acc_helper)
    __guard_hit = __guard_hit and ___check_obj_id(G['torch'], 4317643216)                       # x = a / (torch.abs(a) + 1)  # data/DeepLearning/depyf/tests/test_pytorch/test_pytorch.py:4 in toy_function
    __guard_hit = __guard_hit and ___check_obj_id(G['torch'].abs, 4319207856)                   # x = a / (torch.abs(a) + 1)  # data/DeepLearning/depyf/tests/test_pytorch/test_pytorch.py:4 in toy_function
    return __guard_hit

# Note: please refer to the graph code in __compiled_fn_3_221467c0_09ce_42df_9b0d_2b9dd18ef572*.py.
# Captured Graph: Dynamo generated graph (debuggable when using eager backend).
# Joint graph: joint forward+backward graph from aot autograd.
# Forward graph: forward graph from aot autograd (debuggable when using aot_eager backend).
# Backward graph: backward graph from aot autograd (debuggable when using aot_eager backend).
# AFTER XXX: graph processed by inductor (not debuggable).
def __compiled_fn_3_221467c0_09ce_42df_9b0d_2b9dd18ef572(*args, **kwargs):
    pass
...
def __transformed_code_3_for_toy_function(a, b):
    ...
    graph_out_0 = __compiled_fn_3_221467c0_09ce_42df_9b0d_2b9dd18ef572(
...
___check_tensors = '''None'''
___check_tensors_verbose = '''None'''
___check_global_state = '''<built-in method check of torch._C._dynamo.guards.GlobalStateGuard object at 0x1386c75f0>'''
___check_torch_function_mode_stack = '''<function make_torch_function_mode_stack_guard.<locals>.check_torch_function_mode_stack at 0x13b03e480>'''
IsNonOverlappingAndDenseIndicator = '''<function eval_is_non_overlapping_and_dense at 0x11df50a40>'''
cast_symbool_to_symint_guardless = '''<function cast_symbool_to_symint_guardless at 0x11df50c20>'''
math = '''<module 'math' (built-in)>'''
torch = '''<module 'torch' from '/Users/youkaichao/uv_envs/py312/lib/python3.12/site-packages/torch/__init__.py'>'''
...
# Note: please refer to the graph code in __compiled_fn_1_64010be2_b726_4c83_8d8d_6a5ad75e6da4*.py.
# Captured Graph: Dynamo generated graph (debuggable when using eager backend).
# Joint graph: joint forward+backward graph from aot autograd.
# Forward graph: forward graph from aot autograd (debuggable when using aot_eager backend).
# Backward graph: backward graph from aot autograd (debuggable when using aot_eager backend).
# AFTER XXX: graph processed by inductor (not debuggable).
def __compiled_fn_1_64010be2_b726_4c83_8d8d_6a5ad75e6da4(*args, **kwargs):
    pass
...
def __transformed_code_0_for_toy_function(a, b):
    ...
    __import_torch_dot__dynamo_dot_utils.record_pregraph_bytecode_exit(tmp_2)
    graph_out_0 = __compiled_fn_1_64010be2_b726_4c83_8d8d_6a5ad75e6da4(a, b)
    []
    __temp_7 = []
    __temp_7.extend([])
    [__temp_7]
