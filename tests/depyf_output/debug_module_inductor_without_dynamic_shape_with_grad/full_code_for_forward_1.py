...
___check_global_state = '''<built-in method check of torch._C._dynamo.guards.GlobalStateGuard object at 0x11d7a1df0>'''
___check_torch_function_mode_stack = '''<function make_torch_function_mode_stack_guard.<locals>.check_torch_function_mode_stack at 0x11dd71d00>'''
IsNonOverlappingAndDenseIndicator = '''<function eval_is_non_overlapping_and_dense at 0x119654a40>'''
cast_symbool_to_symint_guardless = '''<function cast_symbool_to_symint_guardless at 0x119654c20>'''
...
___key_to_id = '''<function key_to_id at 0x119798540>'''
___dict_version = '''<built-in function dict_version>'''
___dict_contains = '''<function _get_closure_vars.<locals>.<lambda> at 0x11de5c720>'''
___tuple_iterator_len = '''<method '__length_hint__' of 'tuple_iterator' objects>'''
___normalize_range_iter = '''<function normalize_range_iter at 0x11978fe20>'''
___tuple_iterator_getitem = '''<function tuple_iterator_getitem at 0x11978fce0>'''
___dataclass_fields = '''<function dataclass_fields at 0x11978fd80>'''
___namedtuple_fields = '''<function _get_closure_vars.<locals>.<lambda> at 0x11de5c7c0>'''
___get_torch_function_mode_stack_at = '''<function get_torch_function_mode_stack_at at 0x1197a00e0>'''
__math_isnan = '''<built-in function isnan>'''
__numpy_isnan = '''<ufunc 'isnan'>'''
inf = '''inf'''
__load_module = '''<function import_module at 0x102952840>'''
...
___from_numpy = '''<function from_numpy at 0x119c60ae0>'''
___as_tensor = '''<function _as_tensor_fullprec at 0x10f3d6e80>'''
...
def __guard_1_for_forward(L, G, **___kwargs_ignored):
    ...
    __guard_hit = __guard_hit and ___check_obj_id(G['torch'], 4345053568)                       # x = a / (torch.abs(a) + 1)  # data/DeepLearning/depyf/tests/test_pytorch/test_pytorch.py:13 in forward
    __guard_hit = __guard_hit and ___check_obj_id(G['torch'].abs, 4346617968)                   # x = a / (torch.abs(a) + 1)  # data/DeepLearning/depyf/tests/test_pytorch/test_pytorch.py:13 in forward
    return __guard_hit

# Note: please refer to the graph code in __compiled_fn_1_a3408d5e_72f6_4d33_b7f3_ce0f60cff831*.py.
# Captured Graph: Dynamo generated graph (debuggable when using eager backend).
# Joint graph: joint forward+backward graph from aot autograd.
# Forward graph: forward graph from aot autograd (debuggable when using aot_eager backend).
# Backward graph: backward graph from aot autograd (debuggable when using aot_eager backend).
# AFTER XXX: graph processed by inductor (not debuggable).
def __compiled_fn_1_a3408d5e_72f6_4d33_b7f3_ce0f60cff831(*args, **kwargs):
    pass
