...
Ever felt overwhelmed by the complexities of `torch.compile`? Diving into its workings can feel like black magic, with bytecode that many Python users fail to understand. This hinders users from understanding and debugging `torch.compile`.

We are excited to announce `depyf`, a new tool to pull out all the artifacts of `torch.compile`, and to decompile all the bytecode into source code so that every Python users understand it.
...
Nightly code (recommended): `pip install git+https://github.com/thuml/depyf.git`
...
Any discussion/issue report/PR is welcome. Or contact youkaichao@gmail.com if you have any other problem.
